...
buildenv_img="ldmx-buildenv${IMGMARCH}-el${CENTOS}-c${CMAKE_VER}-d${DEVTOOLSET}-x${XERCES_VER}-g${GEANT4_TAG}-r${ROOTVERSION}-onnx${ONNX_VER}-gpu${ONNX_GPU}"

if [ ! -f "$buildenv_img" ]; then
  sudo singularity build --sandbox ldmx-dev-test/ def/ldmx-buildenv-dev.def
fi

#ln -sf "$buildenv_img" ldmx-buildenv.sif

# do not include XERCES and CMAKE version as we are not going to change those
#ldmx_img="ldmx-${LDMX_TAG}${IMGMARCH}-g${GEANT4_TAG}-r${ROOTVERSION}-el${CENTOS}-d${DEVTOOLSET}.sif"
